...
// Chunk size = 2500 chars (via CHUNK.DEFAULT_SIZE from config). This matches
// the setting used in nodejs/src/local-app/server.ts where recall was tuned
// on legal documents and gave strong results. The previous value (800) starved
// GLiNER of context — names like "After Comma LLC" only scored ~0.345 in a
// 262-char chunk of signature block because the surrounding "Registered
// address:", "(the 'Company')" and contact info were in separate chunks. With
// 2500-char chunks those signals are all co-located and scores rise to
// 0.5-0.7, clearing the 0.30 threshold reliably.
//
// Latency: per-call inference on fp32 GLiNER at 2500 chars is ~1s on CPU
// (vs ~300ms at 800 chars). Total time per document is comparable because
// we make proportionally fewer calls — 10 × 1s ≈ 32 × 0.3s. Per-call 1s is
// still ≪ the 20s/60s tool-call budget, so no timeout risk.
const NER_CHUNK_SIZE = CHUNK.DEFAULT_SIZE;   // 2500
const NER_CHUNK_OVERLAP = 300;               // ~12% of chunk, scaled with chunk size
