...
export function filterFalsePositives(entities: DetectedEntity[], text?: string): DetectedEntity[] {
  ...
  for (const e of entities) {
    ...
    // IMPORTANT: skip single-word tails. LEGAL_STOPLIST contains corporate suffixes as
    // standalone entries ("llc", "ltd", "inc", "corp", "plc", "gmbh", "sarl", "llp",
    // "limited", "incorporated", "corporation") so entities can detect a bare suffix
    // as not-PII. But those same suffixes are LEGITIMATE parts of real company names
    // ("Acme LLC", "After Comma LLC", "Thornbridge Capital Holdings Limited"). Without
    // the single-word skip this rule killed every ORG ending with a corp suffix, giving
    // catastrophic ORG recall loss in legal documents. Only multi-word tails indicate
    // a disqualifying phrase like "Program Managers" / "Printed Name".
    {
      const words = txt.trim().split(/\s+/);
      let tailDrop = false;
      if (words.length >= 2 && words.length <= 4) {
        for (let i = 1; i < words.length; i++) {
          const tailWords = words.slice(i);
          if (tailWords.length === 1) continue; // skip single-word tails (corporate suffixes)
          const tail = tailWords.join(" ").toLowerCase();
