...
export async function extractPdfText(filePath: string): Promise<string> {
  ...
  try {
    ...
    for (const page of data.pages) {
      ...
      // `utils` exists on PDFExtract at runtime but isn't in the .d.ts typings.
      const lines = (PDFExtract as unknown as { utils: { pageToLines: (page: unknown, tol: number) => unknown[] } }).utils.pageToLines(page, 2);
